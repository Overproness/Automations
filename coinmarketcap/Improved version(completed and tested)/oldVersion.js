...
// New function to get a temporary email (with retry and fallback mechanisms)
async function getTempEmail(browser, proxy) {
  ...
  for (const provider of providers) {
    try {
      ...
      await page.waitForSelector(provider.emailSelector, { timeout: 15000 });
      ...
      if (provider.name === "temp-mail.org") {
        ...
        await sleep(5000); // Short wait for other providers
      }
      ...
      try {
        ...
        await page.waitForSelector(provider.copyBtnSelector, { timeout: 5000 });
        await page.click(provider.copyBtnSelector);
        console.log("✅ Clicked copy button using CSS selector");
      } catch (e) {
        try {
          // Then try XPath
          const [copyButton] = await page.$x(provider.copyBtnXPath);
          if (copyButton) {
...
// Main account creation process - with fixed verification flow for the onboarding screens
async function createAccount(proxy) {
  ...
  try {
    ...
    const { email, page: emailPage, provider } = tempEmailData;
    ...
    for (let attempt = 1; attempt <= maxSignupAttempts; attempt++) {
      try {
        ...
        try {
          ...
          await cmcPage.waitForSelector('button[data-role="login"]', {
            timeout: 15000,
          });
          await cmcPage.click('button[data-role="login"]');
        } catch (error) {
          console.error("❌ Error clicking login button:", error);
          // Try XPath
          try {
            const [menuButton] = await cmcPage.$x(
        ...
        try {
          ...
          const [rejectButton] = await cmcPage.$x(
        ...
        try {
          ...
          // Try XPath
          try {
            const [signupButton] = await cmcPage.$x(
        ...
        try {
          ...
          // Try XPath
          try {
            const [emailInput] = await cmcPage.$x(
        ...
        try {
          ...
          // Try XPath
          try {
            const [passwordInput] = await cmcPage.$x(
        ...
        try {
          ...
          // Try XPath
          try {
            const [submitButton] = await cmcPage.$x(
    ...
    try {
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                const [nextButton] = await verificationPage.$x(
      ...
      try {
        ...
        try {
          ...
          const [nicknameInput] = await verificationPage.$x(
          ...
          if (nicknameInput) {
            ...
            // Type nickname
            const localPart = email.split("@")[0];
            await nicknameInput.type(localPart);
      ...
      try {
        ...
        const [finalButton] = await verificationPage.$x(
        ...
        if (finalButton) {
          ...
          // Try div[6] XPath
          const [altButton] = await verificationPage.$x(
            "/html/body/div[6]/div/div/div/div/button",
          );
          if (altButton) {
